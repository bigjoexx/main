...
export default function init() {
    ...
    loader.load(
        ...
        (gltf) => {
            ...
            animations.forEach(name => {
                ...
                action.clampWhenFinished = true; // This will make the animation stay at its last frame when completed.
                actions.push(action);
            });
    ...
    let currentSliderValue = 0; // To keep track of the current slider value

// Listen for the custom event from the slider
window.addEventListener('sliderUpdated', function(e) {
    const newSliderValue = e.detail.percentage;
    
    if (newSliderValue < currentSliderValue) {
        // Slider moved backward
    ...
    currentSliderValue = newSliderValue; // Update the current slider value
});
...
mixer.addEventListener('finished', (event) => {
    ...
    if (actionIndex !== -1) {
        actions[actionIndex].stop(); // Ensuring that the animation stops at the end
    }
});
