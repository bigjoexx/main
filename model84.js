...
export default function init() {
    ...
    // Disable pointer events on the canvas to make it non-interactable
    ...
    controls.enabled = false; // Disabling the controls
...
setTimeScaleBasedOnBreakpoints();
    ...
    const startPercentages = [0, 0.37, 0.32, 0.45];  // Example percentages
...
window.addEventListener('sliderUpdated', (e) => {
    ...
    if (isMovingForward) {
        ...
        if (actionIndex !== -1) {
            actions[actionIndex].paused = false;  
            actions[actionIndex].timeScale = positiveTimeScale;
            
            // Start the animation from the specific percentage of its duration
            actions[actionIndex].time = startPercentages[actionIndex] * actions[actionIndex]._clip.duration;
