...
export default function init() {
    ...
    if (event.action.timeScale === 3) { // If the animation was playing forward
    ...
    } else if (event.action.timeScale === -3) { // If the animation was playing in reverse
        event.action.paused = true; // Pause the action
...
window.addEventListener('sliderUpdated', (e) => {
    ...
    if (isMovingForward) {
        ...
        if (actionIndex !== -1) {
            ...
            actions[actionIndex].timeScale = 3;   // Play forward
        ...
        if (actionIndex !== -1) {
            actions[actionIndex].paused = false;  // Ensure the animation isn't paused
            actions[actionIndex].timeScale = -3;  // Play backward
