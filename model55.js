...
window.addEventListener('sliderUpdated', function(e) {
    ...
    let actionToPlay;
    let playInReverse = newSliderValue < currentSliderValue; // Determine direction

    if (playInReverse) {
        // If slider is moving in reverse, use these steps/values
        ...
        // If slider is moving forward, use these steps/values
    ...
    if (actionToPlay) {
        actionToPlay.reset();
        actionToPlay.setDirection(playInReverse ? -1 : 1); // Set direction based on if we're playing in reverse
        actionToPlay.play();
    }
