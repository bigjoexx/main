...
export default function init() {
    ...
    // Convert the received value to an integer for comparison
    const value = parseInt(e.detail.percentage);
    const index = values.indexOf(value);

    if (index !== -1) {
        console.log(`Playing action at index ${index}`);
        actions[index].play();
    } else {
        console.warn(`No action found for value ${value}`);
    }
});
